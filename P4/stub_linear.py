...
class Learner(object):
    ...
    def state_transformation(self, state):
        # extract relevant normalized state features
        state_list = [(state['monkey']['vel']+40.)/80.]
        state_list.append((state['monkey']['top']-57.)/343.)
        state_list.append((state['tree']['dist']+120.)/583.)
        state_list.append((state['tree']['top']-200.)/200.)
        state_list.append((state['monkey']['vel']-self.last_velocity+80.)/160.)
